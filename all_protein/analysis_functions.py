...
def place_all_wat(all_coord_info, s,
    ...
    norm_all = []
    resi_list = []
    s.extract('name', 'OTX', '==').name = 'O' #replace naming for terminal oxygen
    for r in list(s.residues):
        if r.resn[0] in list(DICT4A.keys()):
            resi_list.append(r)
    ...
    #for r in resi_list:
        #print(r)
        #get_new_all_coords_parallel(all_coord_info, r, min_ang, cont_dict, cutoff_idx, all_xyz_coords, norm_list, s, use_cutoff)
    with Pool(processes=8) as p:        
        pool_output = p.starmap(get_new_all_coords_parallel, main_inputs)
    out_coords = [result[0] for result in pool_output]
    norm = [result[1] for result in pool_output]
    out_coords_all = np.concatenate(out_coords)
    norm_all = np.concatenate(norm)
...
def get_new_all_coords_parallel(all_coord_info, 
                   res, 
                   min_ang, 
                   cont_dict,
                   cutoff_idx,
    ...
    print(res.resn[0])
    print(res.resi[0])
    #if res.resn[0] not in ROTAMERS: #not an amino acid
    #   continue
    ...
    all_altlocs = set(res.altloc)
    print(f'all altlocs: {all_altlocs}')
    if len(all_altlocs) > 1: 
      try:
        all_altlocs.remove('')
      except ValueError:
        pass
    print(f'all_altlocs 2: {all_altlocs}')
    for a in all_altlocs:
        ...
        for atom_set_spec, atom_set_gen in DICT4A[res.resn[0]].items(): # go through each set of 4 atoms in this residue
            ...
            if 'YR1' in atom_set_gen or 'FR1' in atom_set_gen or 'WR1' in atom_set_gen or 'PR1' in atom_set_gen or 'HR1' in atom_set_gen:
                ...
                for at in atom_set_spec:
                    if len(at)>2: # if part of ring
                        ...
                        print(name2idx)
                        print(res.resi[0])
                        print(f'name2idx[at]: {name2idx[at]}')
                        print(res.coor[name2idx[at]])
            ...
            # otherwise determine the right bucket
                ...
                #print(type(closest_min_idx))
                if diff_from_mins[closest_min_idx]>0:
                    bucket = np.append(bucket, (closest_min_idx+1))
                else:
                    bucket = np.append(bucket,(closest_min_idx))
